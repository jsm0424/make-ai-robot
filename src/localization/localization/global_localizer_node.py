...
By combining the odom localization and global localization, we can get a ac
curate localization.
odom_localizer: tf from odom to base frame
global_localizer: tf from map to odom frame
combined: tf from map to base frame
Current code only publishes the initial pose from launch file as odom fram
e.
You need to modify this node to use map and other sensors to localize the r
obot.
"""
import rclpy
